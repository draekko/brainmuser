            ...
            if (activity != null) {
                activity.runOnUiThread(new Runnable() {
                    ...
                    public void run() {
                        ...
                        Double leftEEG = data.get(Eeg.TP9.ordinal()) + data.get(Eeg.FP1.ordinal());
                        Double rightEEG = data.get(Eeg.TP10.ordinal()) + data.get(Eeg.FP2.ordinal());
                        SurfaceView leftView = (SurfaceView) findViewById(R.id.left);
                        SurfaceView rightView = (SurfaceView) findViewById(R.id.right);
                        int upperLimit = 1700;
                        //                      r   g   b       r  g   b
                        //we need to go from    FF  00  00 to   00 00  FF
                        //FF0000    000000  0000FF
                        /*if (cur <0) {
                            if (leftEEG > upperLimit) {
                                leftView.setBackgroundColor(cur);
                                cur += 100000;
                                String a = ""+cur;
                                Log.i("Color Number: ",a);
                        ...
                        int difference = (int) (rightEEG - leftEEG);//so this is how much greater right is. I've checked, range is usually within plus minus 1000
                        Log.i("DJP", "" + difference);
                        TextView brain = (TextView) findViewById(R.id.brain);
                        if (difference < 900 && difference > -900) {
                            int color = Color.rgb(0, 0, difference);
                            int opcolor = Color.rgb(0, 0, -difference);
                            rightView.setBackgroundColor(color);
                            leftView.setBackgroundColor(opcolor);


                        }
                        if(brain.getPaddingRight()<1600){
                            if (rightEEG > 1800) {
                                brain.setPadding(0, 0, brain.getPaddingRight() + 1, 0);
                            }
                        }
                        else{
                            if(leftEEG>1800){
                                brain.setPadding(0,0,1600,brain.getPaddingBottom()+1);
                            }
                            if(brain.getPaddingBottom()>1600){
                                brain.setPadding(0,0,1600,1600);
                            }
                        }
            ...
            if (activity != null) {
                activity.runOnUiThread(new Runnable() {
                    ...
                    public void run() {
                        ...
                        TextView tp10 = (TextView) findViewById(R.id.eeg_tp10);
                        tp9.setText(String.format(
                                "%6.2f", data.get(MuseDataPacketType.ALPHA_ABSOLUTE.ordinal())));
                        fp1.setText(String.format(
                                "%6.2f", data.get(Eeg.FP1.ordinal())));
                        fp2.setText(String.format(
                                "%6.2f", data.get(Eeg.FP2.ordinal())));
